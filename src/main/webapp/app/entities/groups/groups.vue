<template>
  <div>
    <h2 id="page-heading" data-cy="GroupsHeading">
      <span v-text="$t('rnApp.groups.home.title')" id="groups-heading">Groups</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('rnApp.groups.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'GroupsCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('rnApp.groups.home.createLabel')"> Create a new Groups </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && groups && groups.length === 0">
      <span v-text="$t('rnApp.groups.home.notFound')">No groups found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="groups">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('rnApp.groups.name')">Name</span></th>
            <th scope="row"><span v-text="$t('rnApp.groups.employee')">Employee</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="groups in groups" :key="groups.id" data-cy="entityTable">
            ...
            <td>{{ groups.name }}</td>
            <td>
              <span v-for="(employee, i) in groups.employees" :key="employee.id"
                >{{ i > 0 ? ', ' : '' }}
                <router-link class="form-control-static" :to="{ name: 'EmployeeView', params: { employeeId: employee.id } }">{{
                  employee.id
                }}</router-link>
              </span>
            </td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="rnApp.groups.delete.question" data-cy="groupsDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-groups-heading" v-text="$t('rnApp.groups.delete.question', { id: removeId })">
