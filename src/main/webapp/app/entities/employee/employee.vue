<template>
  <div>
    <h2 id="page-heading" data-cy="EmployeeHeading">
      <span v-text="$t('rnApp.employee.home.title')" id="employee-heading">Employees</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('rnApp.employee.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EmployeeCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('rnApp.employee.home.createLabel')"> Create a new Employee </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && employees && employees.length === 0">
      <span v-text="$t('rnApp.employee.home.notFound')">No employees found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="employees">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('firstName')">
              <span v-text="$t('rnApp.employee.firstName')">First Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('lastName')">
              <span v-text="$t('rnApp.employee.lastName')">Last Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('email')">
              <span v-text="$t('rnApp.employee.email')">Email</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'email'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('phoneNumber')">
              <span v-text="$t('rnApp.employee.phoneNumber')">Phone Number</span>
            ...
            <th scope="row" v-on:click="changeOrder('hireDate')">
              <span v-text="$t('rnApp.employee.hireDate')">Hire Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('region.id')">
              <span v-text="$t('rnApp.employee.region')">Region</span>
        ...
        <tbody>
          <tr v-for="employee in employees" :key="employee.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="employee.region">
                <router-link :to="{ name: 'RegionView', params: { regionId: employee.region.id } }">{{ employee.region.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="rnApp.employee.delete.question" data-cy="employeeDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-employee-heading" v-text="$t('rnApp.employee.delete.question', { id: removeId })">
